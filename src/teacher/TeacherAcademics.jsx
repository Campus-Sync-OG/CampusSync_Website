    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        const list = Array.isArray(res)
          ? res
          : res.notifications || res.data || res;
        if (!Array.isArray(list)) return;

        const found = list.find((n) => {
          try {
            const parsed =
              typeof n.message === "string" ? JSON.parse(n.message) : n.message;
            ...
            if (parsed.submission_ref_notification_id && meta.notificationId) {
              return (
                String(parsed.submission_ref_notification_id) ===
                String(meta.notificationId)
              );
            }
            ...
            if (parsed.submission_summary) {
              ...
              return (
                ...
                s.class_grade === meta.class_grade &&
                s.section === meta.section &&
                s.exam_format === meta.exam_format
              );
            }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const notificationId =
        res?.notificationId ||
        (Array.isArray(res?.notificationIds) ? res.notificationIds[0] : null);
        ...
        <>
          ...
          >
              ...
              reviewComments && (
                <div
                  style={{
                    marginLeft: 12,
                    color:
                      submissionStatus === "approved" ? "#2e7d32" : "#e53935",
                  }}
                >
            ...
            <Button
              ...
              style={{ background: "#0069c0" }}
            >
              Check status
            </Button>
            {polling && (
              <div style={{ marginLeft: 8, color: "#666" }}>
                Waiting for principal reply…
              </div>
            )}
          </div>
        </>
